...
class Weather:
    ...
    def __init__(self, base_url="https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/", api_key=''):
        """
    ...
    def fetch_weather_data(self, location, from_date='', to_date='', unit_group='metric', include='days', elements=''):
        """
        Fetch weather data for a specified location and date range.

        Parameters:
            location (str): Location for which weather data is requested.
            from_date (str): Start date of the weather data period.
            to_date (str): End date of the weather data period.
            unit_group (str): Unit system for the weather data ('metric' or 'us').
    ...
    def get_data_on_day(self, day_info):
        """
        ...
        try:
            if isinstance(day_info, str):
                return next(day for day in self.__weather_data.get('days', []) if day['datetime'] == day_info)
            elif isinstance(day_info, int):
                return self.__weather_data['days'][day_info]
            else:
                raise ValueError(f"Invalid input value for get_data_on_day() with str or int: {day_info}")
        except Exception as e:
            raise e
    ...
    def set_item_by_datetimeVal(src, datetimeVal, data):
        ...
        Parameters:
            ...
            data (dict): The new data dictionary to replace the old dictionary; must at least contain 'datetime'.
    ...
    def get_data_at_datetime(self, day_info, time_info):
        """
        Retrieves weather data for a specific date and time from the weather data collection.

        Parameters:
            day_info (str|int): A day identifier, which can be a date string or an index, pointing to a specific day in the data.
            time_info (str|int): A time identifier, which can be a time string or an index, pointing to a specific time slot in the day's data.
        ...
        try:
            ...
            return Weather.filter_item_by_datetimeVal(day_item['hours'], time_info)
        except Exception as e:
            raise e
    ...
    def get_datetimeEpoch_at_datetime(self, day_info, time_info):
        ...
        Parameters:
            day_info (str|int): A day identifier, which can be a date string or an index.
            time_info (str|int): A time identifier, which can be a time string or an index.
...
if __name__ == "__main__":
    ...
    # print(">>Result: ", weather.get_weather_data())

    # using the method [get_tzoffset] of the class to get the `tzoffset`
    print(">>tzoffset: ", weather.get_tzoffset())

    # using the method [set_tzoffset] of the class to set the `tzoffset`
    ...
    # print(">>Weather data in a day: ", weather.get_data_on_day('2023-01-02'))

    # using the method [get_hourlyData_on_day] of the class to get hourly data of the specific day within `days` key
    print(">>Hourly data with some elements in a day: ", weather.get_hourlyData_on_day('2023-01-02', ['temp', 'humidity']))

    # using the method [get_tempmax_on_day] of the class to get tempmax of the specific day within `days` key
    print(">>tempmax in a day: ", weather.get_tempmax_on_day(1))
    print(">>tempmax in a day: ", weather.get_tempmax_on_day('2023-01-02'))
    ...
    print(">>changed tempmax in a day: ", weather.get_tempmax_on_day(1))
    ...
    print(">>snowdepth in a day: ", weather.get_snowdepth_on_day(1))
    weather.set_snowdepth_on_day('2023-01-02', 1)
    print(">>changed snowdepth in a day: ", weather.get_snowdepth_on_day(1))
    ...
    # print(">>Weather data in a datetime: ", weather.get_data_at_datetime(1,1))
    # weather.set_data_at_datetime('2023-01-02', 1, {'temp': 25.0})
    # print(">>Changed weather data in a datetime: ", weather.get_data_at_datetime('2023-01-02', '01:00:00'))
    ...
    print(">>datetimeEpoch in a datetime: ", weather.get_datetimeEpoch_at_datetime(1,1))
    weather.set_datetimeEpoch_at_datetime('2023-01-02', 1, '1111111111')
    print(">>changed datetimeEpoch in a datetime: ", weather.get_datetimeEpoch_at_datetime('2023-01-02', '01:00:00'))
    ...
    print(">>stations in a datetime: ", weather.get_stations_at_datetime(1,1))
